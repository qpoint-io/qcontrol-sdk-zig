...
pub fn build(b: *std.Build) void {
    ...
    // Net logger plugin - shared library
    const net_logger = b.addLibrary(.{
        .linkage = .dynamic,
        .name = "net_logger",
    ...
    net_logger.root_module.addImport("qcontrol", qcontrol_mod);
    b.installArtifact(net_logger);

    // Net logger plugin - object file for bundling
    const net_logger_obj = b.addObject(.{
        .name = "net_logger",
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/main.zig"),
            .target = target,
    ...
    net_logger_obj.root_module.addImport("qcontrol", qcontrol_mod);
    const install_obj = b.addInstallFile(net_logger_obj.getEmittedBin(), "lib/net_logger.o");
