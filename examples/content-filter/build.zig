...
pub fn build(b: *std.Build) void {
    ...
    // Content filter plugin - shared library
    const content_filter = b.addLibrary(.{
        .linkage = .dynamic,
        .name = "content_filter",
    ...
    content_filter.root_module.addImport("qcontrol", qcontrol_mod);
    b.installArtifact(content_filter);

    // Content filter plugin - object file for bundling
    const content_filter_obj = b.addObject(.{
        .name = "content_filter",
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/main.zig"),
            .target = target,
    ...
    content_filter_obj.root_module.addImport("qcontrol", qcontrol_mod);
    const install_obj = b.addInstallFile(content_filter_obj.getEmittedBin(), "lib/content_filter.o");
