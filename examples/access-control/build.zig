...
pub fn build(b: *std.Build) void {
    ...
    // Access control plugin - shared library
    const access_control = b.addLibrary(.{
        .linkage = .dynamic,
        .name = "access_control",
    ...
    access_control.root_module.addImport("qcontrol", qcontrol_mod);
    b.installArtifact(access_control);

    // Access control plugin - object file for bundling
    const access_control_obj = b.addObject(.{
        .name = "access_control",
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/main.zig"),
            .target = target,
    ...
    access_control_obj.root_module.addImport("qcontrol", qcontrol_mod);
    const install_obj = b.addInstallFile(access_control_obj.getEmittedBin(), "lib/access_control.o");
